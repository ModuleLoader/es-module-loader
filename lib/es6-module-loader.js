 ...
 */
 /*globals exports, ActiveXObject, require */
(function () {
  (function() {

    var isBrowser = typeof window !== 'undefined',
      global = isBrowser ? window : exports;
    ...
    function Loader(options) {
      ...
      var i,
        wrapped = {},
        wrap = isBrowser ? ['addEventListener', 'removeEventListener', 'getComputedStyle', 'setTimeout', 'setInterval'] : [];
    ...
    Loader.prototype.load = function (url, callback, errback) {
      var scriptCnt, i, _callback, _errback,
        self = this;
    ...
    Loader.prototype['import'] = function (name, callback, errback, referer) {
      var i, modules, moduleCnt, self = this;
      ...
      var _callback = function(module) {
            ...
            for (i = 0; i < self._mloads[name].length; i++) {
    ...
    Loader.prototype._linkExecute = function (name, source, opt, callback, errback) {
      ...
      var self,
        isScript = opt.type == 'script',
        link = this.link(source, opt);
      ...
      ES6Parser.loadEsprima(name, source, function() {
        ...
        for (i = 0; i < imports.length; i++) {
          (function(i) {
            referer = { name: name, address: opt.address };
    ...
    var defaultSystemLoader = new Loader({
      ...
      resolve: function (name, options) {
        var r, i, tablePropLen;
        for (r in this.ondemandTable) {
          if (Array.prototype.indexOf) {
            ...
            for (i = 0, tablePropLen = this.ondemandTable[r].length; i < tablePropLen; i++) {
              if (this.ondemandTable[r][i] === name) {
                return name;
              }
            }
          }
