...
(function () {
    ...
    var defaultSystemLoader = new Loader({
      ...
      translate: function (source, options) {
        ...
        /* if (traceur.options.sourceMaps) {
          ...
          opt.sourceMapGenerator = sourceMapGenerator;
        } */

        source = traceur.outputgeneration.ProjectWriter.write(res, opt);

        // NB fix source map support here
  ...
  var __scopedEval = function(__source, global, __sourceURL) {
    var __sourceMappingURL, __existingSourceURL;
    if (__sourceMappingURL = __source.match(/\/\/[@#] ?sourceMappingURL=(.+)/m))
      __sourceMappingURL = __sourceMappingURL[1];
    else if (__existingSourceURL = __source.match(/\/\/[@#] ?sourceURL=(.+)/m))
      __sourceURL = __existingSourceURL[1];


    eval('(function(window) { with(global) { ' + __source + ' } }).call(global, global);' + (__sourceMappingURL 
