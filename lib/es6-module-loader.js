  ...
  (function() {
    ...
    var ES6Parser = {
      ...
      loadTraceur: function(name, source, callback, errback) {
        ...
        global.System.load(traceurSrc, function() {
      ...
      getTransformedSyntaxTree: function(source, options) {
        ...
        this.traceur.options.sourceMaps = true;
        this.traceur.options.modules = 'parse';

        var reporter = new this.traceur.util.ErrorReporter();
        reporter.reportMessageInternal = function(location, kind, format, args) {
          throw kind + '\n' + location;
        }

        var sourceFile = new this.traceur.syntax.SourceFile(options.address, source);
        var parser = new this.traceur.syntax.Parser(reporter, sourceFile);
        var tree = options.type == 'module' ? parser.parseModule() : parser.parseScript();

        var transformer = new this.traceur.codegeneration.ProgramTransformer(reporter, project);
        
        return this.treeCache[name] = transformer.transform(tree);
      },
      // parse the list of import module names for a given source
      parseImports: function(source, options) {
        var tree = this.getTransformedSyntaxTree(source, options);

        // no tree means no syntax parse necessary
        if (!tree)
          return [];
      ...
      parseEval: function(source, loader, opt) {
        ...
        var tree = this.getTransformedSyntaxTree(source, { normalized: opt.name, address: opt.address });

        var normalizeMap = opt.normalizeMap || {};

        // generate source
        var sourceMapGenerator = new this.traceur.outputgeneration.SourceMapGenerator({ file: opt.address });
        ...
        this.traverse(tree, function(node) {
          ...
          if (node.type == 'IMPORT_DECLARATION') {
            var moduleName = normalizeMap[node.moduleSpecifier.token.processedValue] || node.moduleSpecifier.token.processedValue;
            ...
            // import { ... } from 'jquery';
              ...
              var specifiers = node.importSpecifierSet.specifiers;
              for (var i = 0; i < specifiers.length; i++) {
                var specifier = specifiers[i];
                replaceSource += "var " + (specifier.rhs ? specifier.rhs.value : specifier.lhs.value) + " = __module['" + (specifier.rhs ? specifier.lhs.value : specifier.lhs.value) + "'];";
              }
              // console.log(node.location);
              tSource.replace(node.location.start.offset, node.location.end.offset, replaceSource);
              console.log(tSource.toString());
            }
          }

          // --- Exports ---
          else if (node.type == 'EXPORT_DECLARATION') {
            ...
            if (declaration.type == 'NAMED_EXPORT') {
              if (!declaration.moduleSpecifier) {
                ...
                for (var i = 0; i < specifiers.length; i++) {
                  var specifier = specifiers[i];
                  replaceSource += "__exports['" + (specifier.rhs ? specifier.rhs.value : specifier.lhs.value) + "'] = " + (specifier.rhs ? specifier.lhs.value : specifier.lhs.value) + "; ";
                }
                
                tSource.replace(node.location.start.offset, node.location.end.offset - 1, replaceSource);
              }
            ...
            // export var p = 4;
              ...
              var exportName, declarationIndex = declaration.location.start.offset - 1;

              if (declaration.type == 'VARIABLE_STATEMENT') {
                var varDeclaration = declaration.declarations.declarations[0];
                
                // export var p = ...
                exportName = varDeclaration.lvalue.identifierToken.value;
                declarationIndex = varDeclaration.lvalue.location.end.offset - 1;
                tSource.replace(node.location.start.offset, declarationIndex, "__exports['" + exportName + "']");
              }
              else if (declaration.type == 'FUNCTION_DECLARATION') {
                exportName = declaration.name.identifierToken.value;
                tSource.replace(node.location.start.offset, declarationIndex, "__exports['" + exportName + "'] = ");
              }
          ...
          // module x from 'y';
          /*else if (node.type == 'MODULE_DECLARATION') {
            tSource.replace(node.range[0], node.range[1], "var " + node.id.name + " = __Loader.get('" + (normalizeMap[node.source.value] || node.source.value) + "');");
          }*/
        });
